#ifndef _RTMPCHUNK_H_
#define _RTMPCHUNK_H_
...
public:
    ...
    int CreatChunk(uint32_t csid, RtmpMessage& in_msg, char* buf, uint32_t buf_size);
...
protected:
    ...
    int CreatBasicHeader(uint8_t fmt, uint32_t csid, char* buf);
    int CreatMessageHeader(uint8_t fmt, RtmpMessage& rtmp_msg, char* buf);
private:
    State state_;
